"""
    演示使用INT8为量化和FP16的混合精度训练

        基座的原始精度: FP16

        非离群值使用INT8量化, 离群值还是使用FP16(不进行量化操作)


    操作

        1. 加载模型使用 load_in_8bit=True 进行8为量化加载, 注意torch_dtype=torch.half还是需要设置为半精度

            尝试打印参数信息, 可以发现绝大部分参数都是torch.int8, 少部分参数如layerNorm层的参数还是torch.float16

            尝试使用print(model.config)输出具体在那些层进行了8位量化参数

            
        2. 模型合并问题

            如何你使用INT8
            混合精度lora、adalora、qlora微调训练, 不要讲微调好的checkpoint和基座模型权重进行合并, 否则容易造成模型效果极差

            
        注意: 实际开发过程中, 不会使用INT8进行训练, 因为它无论微调训练还是SFT训练, 效果极差, 只会在Qlora中使用INT8训练
...
model = AutoModelForCausalLM.from_pretrained(model_dir, low_cpu_mem_usage=True, trust_remote_code=True, device_map="auto", torch_dtype=torch.half, load_in_8bit=True)
