...
# 5.1 使用库中现成的评估指标来进行评估
cls_metrics = evaluate.combine("accuracy", "f1")
...
def evaluate():
    ...
    with torch.inference_mode():                # 推理模式
        for batch in valid_loader:
            ...
            cls_metrics.add_batch(predictions=pred.long(), references=batch["labels"].long())

    return cls_metrics.compute()
...
def train(epochs=3, log_step=100):              # log_step代表每100step输出一个训练信息
    ...
    for epoch in range(epochs):
        ...
        # 每训练完一次epoch进行验证一次
        acc = evaluate()
        print(f"epoch: {epoch} / {epochs + 1}, metrics: {acc}")    
        # 每训练完一个epcoh保存一次
        # model.save_pretrain("")
