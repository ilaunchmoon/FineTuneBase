"""
    设定输入数据的长度来降低显存
        ...
        实现方法:  降低数据集输入llm的最大长度, 将max_length=32等更小的数据来降低训练显存

        注意: 会影响模型效果, 一定要确保最大长度能够大于等于数据的最大长度, 否则很多数据被截断输入LLM, LLM训练效果很差
...
def process_data(example):
    tokenized_data = tokenizer(example["review"], max_length=32, padding="max_length", truncation=True)     # 设置较低的max_length来降低llm训练的显存
