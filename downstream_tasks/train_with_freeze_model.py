"""
    使用Freeze Model方法来实现降低显存

        即将模型中的某些参数都冻结住: 从而降低前向、反向、优化器使用显存的量

        比如: 将模型中的某些层或某些模块的参数冻结中, 如将Bert模型中全连接层的参数冻结住

        实现方法: 在Trainer中实现, 而是在TrainingAgrumetns中设置
...
# 将模型中的某些参数冻结住, 以降低显存使用, 但是这种做法风险很大, 因为会导致很多模型参数不会进行学习
for name, param in model.bert.named_parameters():
    param.requires_grad=False
